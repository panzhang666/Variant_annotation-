...
def get_basic_info(vcfLine):
    ...
    #Reads supporting the variant (AO) versus those supporting reference reads (RO)
    #If there are mutiple alternate allele observations, AO/RO will be calculated for each of them.
    try: 
        if ',' in altDepth:
            varPercent = []
...
#Create a list to store annoations for csv output  
output_list =[]
...
with open(vcfFile, 'r') as vcf:
        ...
        for line in vcf:
            ...
            #Combine the annotation from VCF file and EXaC for csv output 
            #Here, the annotation is appended to a list instead of a dataframe as append to a dataframe is more expensive
            ...
            #Combine the annotation from VCF file and EXaC for VCF output
...
#Append annoation information in\a list to a datafram and then write to a CSV file
