...
"""
Store putative impact for some Sequence Ontology terms to help 
...
def extract_exac_annot(exacInformation, consequenceSeverity):
    ...
    Output: Three dictionaries store "Allele frequency", "Sequence Ontology term", 
...
output_table = pd.DataFrame(columns=("Chrom","Pos","ID", "Type", "Ref", "Alt", "Depth","Num Reads Supporting Variant",
                                    "Variant reads vs Ref reads", "EXaC variant frequency", "Sequence Ontology term", "Related GeneID"  )) 
        ...
        for line in vcf:
            ...
            if line.startswith("#"):
                ...
                if line.startswith("##INFO=<ID=END"):
                    print('##INFO=<ID=ANNOT,Description="Variant annotations. Total read depth | Num Reads Supporting Variant | Variant reads vs Ref reads | EXaC variant frequency | Sequence Ontology term | Related GeneID ">', file=outputVcf)
                continue
            ...
            total_annoation = vcf_info.strip() + "\t" + "\t".join([exacAlleleFreq[position],variantConsequences[position], variantGeneID[position]])
            ...
            output_table.loc[position] = total_annoation.split("\t")
